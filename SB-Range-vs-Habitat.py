...
def download_GAP_habmap_CONUS2001v1(gap_id, toDir):
    ...
    # Get a public item.  No need to log in.
    hab =  items['items'][0]['id']
    item_json = sb.get_item(hab)
    get_files = sb.get_item_files(item_json, toDir)
...
print('\nRunning function to download GAP RANGE from ScienceBase for species code', sppCode,'\n' )
download_GAP_range_CONUS2001v1(sppCode, tempDir)


# Run the download GAP habitat map function
print('\nRunning function to download GAP HABITAT from ScienceBase for species code', sppCode,'\n' )
...
habDBF = glob.glob(tempDir + '*tif.vat.dbf')[0]
...
# This gets the name of the range CSV file which will need to be
#  parsed to get the number of HUCs in the species' range
rangeCSV = glob.glob(tempDir + '*.csv')[0]
# make it a dataframe
...
dfMaster = dfMaster.append(dfMerge)
# Export to CSV
print('Exporting to CSV: SpeciesRangevsHabitat.csv')
